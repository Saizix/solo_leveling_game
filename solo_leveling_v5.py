...
# Vérification si l'inventaire est dans le session_state
if "inventaire" not in st.session_state:
    st.session_state["inventaire"] = {"Potion de Vie": 2, "Potion d'Energie": 1}

# Vérification si les donjons sont dans le session_state
if "donjons" not in st.session_state:
    st.session_state["donjons"] = {"Donjon du Dragon": {"niveau": 1, "xp": 100}, "Caverne Maudite": {"niveau": 2, "xp": 200}}

# Vérification si l'étape du donjon est définie dans le session_state
...
if menu == "🎒 Inventaire":
    ...
    st.write("Inventaire : ", st.session_state["inventaire"])
...
if menu == "🗡️ Donjon":
    ...
    st.write("Donjons disponibles : ", st.session_state["donjons"])
    ...
    if etape_donjon < len(donjon_etapes):
        ...
        if st.button("Accomplir cette étape"):
            if stamina >= cost:
                ...
                xp += xp_gain
                stamina -= cost
                st.session_state["etape_donjon"] += 1
...
st.session_state["inventaire"] = st.session_state["inventaire"]  # Sauvegarde de l'inventaire
st.session_state["donjons"] = st.session_state["donjons"]  # Sauvegarde des donjons
