...
# Exemple de compétences passives
competences_passives = {
    "Régénération": {
        "description": "Rend 5 points d'énergie chaque jour.",
        "actif": True,  # Cette compétence est active
        "effet": lambda: 5  # Rend 5 d'énergie
    },
    "Volonté de Fer": {
        "description": "Booste la Discipline de +10 pendant une heure.",
        "actif": False,  # Inactive sauf si activée
        "effet": lambda: stats["💼 Discipline"] + 10  # Augmente la discipline
    }
}

# Appliquer les compétences passives
def appliquer_competences_passives():
    for competence, details in competences_passives.items():
        if details["actif"]:
            # Appliquer l'effet de la compétence passive
            if competence == "Régénération":
                global energie
                energie += details["effet"]()
                st.success(f"💡 {competence} activée ! {details['description']}")

appliquer_competences_passives()
...
def appliquer_evenements_speciaux():
    # Vérifier quel événement se produit aujourd'hui
...
# Appliquer les événements spéciaux
appliquer_evenements_speciaux()
